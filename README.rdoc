...
If "threshold" option was specified, plugin outputs only when the anomalyscore is more than threshold.
...
If "trend" option was specified, plugin outputs only when the input data tends to up (or down). 
...
If "input_greater_equal" option was specified, plugin outputs only when the current input data is greater than or equal to (>=) the specified value. Notice that the outlier will still be computed for such input data too.
